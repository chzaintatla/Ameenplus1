...
class _HabitsPageState extends ConsumerState<HabitsPage> with SingleTickerProviderStateMixin {
        ...
        children: [
          TabBar(
            ...
            tabs: const [
              ...
              Tab(icon: Icon(Icons.eco), text: 'Tasbih'),
            ],
          ),
          Expanded(
            child: TabBarView(
              controller: _tabController,
              children: [
                _buildHabitsTab(habitsAsync, currentUser),
  ...
  Widget _buildNamazCard(
    ...
    return InkWell(
      onTap: () => _showNamazDialog(context, label, isFarz, userId),
      child: Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(
          color: color.withValues(alpha: 0.1),
  ...
  void _showNamazDialog(BuildContext context, String label, bool isFarz, String userId) {
    int count = 0;
    showDialog(
      context: context,
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text('Add $label Namaz'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(
                'How many $label namaz did you pray?',
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (context.mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('$count $label namaz added!'),
                      backgroundColor: Colors.green,
                    ),
                  );
                }
